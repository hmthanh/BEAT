...
class TextEncoderTCN(nn.Module):
    ...
    def __init__(self, args, n_words, embed_size=300, pre_trained_embedding=None,
        ...
        # use pre-trained embedding (fasttext)
...
class PoseGenerator(nn.Module):
    ...
    def __init__(self, args):
        ...
        if self.facial_f is not 0:
            self.facial_encoder = nn.Sequential(
                BasicBlock(self.facial_dims, self.facial_f//2, 7,
                           1, first_dilation=3,  downsample=True),
                BasicBlock(self.facial_f//2, self.facial_f//2, 3,
                           1, first_dilation=1,  downsample=True),
                BasicBlock(self.facial_f//2, self.facial_f //
                           2, 3, 1, first_dilation=1, ),
                BasicBlock(self.facial_f//2, self.facial_f, 3, 1,
                           first_dilation=1,  downsample=True),
            )
        else:
    ...
    def forward(self, pre_seq, in_audio=None, in_facial=None, in_text=None, in_id=None, in_emo=None, is_test=False):
        ...
        if audio_feat_seq.shape[1] != pre_seq.shape[1]:
            ...
            audio_feat_seq = torch.cat(
                (audio_feat_seq, audio_feat_seq[:, -diff_length:, :].reshape(1, diff_length, -1)), 1)
        ...
        if self.audio_f is not 0 and self.facial_f is 0:
            ...
            in_data = torch.cat(
                (pre_seq, audio_feat_seq, face_feat_seq), dim=2)
        else:
            pass
        ...
        if speaker_feat_seq is not None:
            ...
            if len(repeated_s.shape) == 2:
                repeated_s = repeated_s.reshape(
                    1, repeated_s.shape[1], repeated_s.shape[0])
        ...
        decoder_outputs = output.reshape(
            in_data.shape[0], in_data.shape[1], -1)
    ...
    def forward(self, pre_seq, in_audio=None, in_facial=None, in_text=None, in_id=None, in_emo=None):
        ...
        if self.speaker_embedding:
            ...
            if len(speaker_feat_seq.shape) == 2:
                speaker_feat_seq = speaker_feat_seq.reshape(
                    1, speaker_feat_seq.shape[1], speaker_feat_seq.shape[0])
            speaker_feat_seq = speaker_feat_seq.repeat(1, pre_seq.shape[1], 1)
            in_data = torch.cat((in_data, speaker_feat_seq),
                                2) if in_data is not None else speaker_feat_seq
        ...
        if self.emotion_embedding:
            ...
            emo_feat_seq = self.emotion_embedding_tail(emo_feat_seq)
            emo_feat_seq = emo_feat_seq.permute([0, 2, 1])
            in_data = torch.cat((in_data, emo_feat_seq),
                                2) if in_data is not None else emo_feat_seq

        if in_text is not None:
            text_feat_seq, _ = self.text_encoder(in_text)
            in_data = torch.cat((in_data, text_feat_seq),
                                2) if in_data is not None else text_feat_seq
        ...
        if in_audio is not None:
            ...
            if in_text is not None:
                if (audio_feat_seq.shape[1] != text_feat_seq.shape[1]):
                    ...
                    audio_feat_seq = torch.cat(
                        (audio_feat_seq, audio_feat_seq[:, -min_gap:, :]), 1)

            audio_fusion_seq = self.audio_fusion(torch.cat(
                (audio_feat_seq, emo_feat_seq, speaker_feat_seq, text_feat_seq), dim=2).reshape(-1, self.audio_fusion_dim))
            audio_feat_seq = audio_fusion_seq.reshape(*audio_feat_seq.shape)
            in_data = torch.cat((in_data, audio_feat_seq),
                                2) if in_data is not None else audio_feat_seq
        ...
        if self.facial_f is not 0:
            ...
            if (audio_feat_seq.shape[1] != face_feat_seq.shape[1]):
                ...
                if min_gap_2 > 0:
                    face_feat_seq = face_feat_seq[:,
                                                  :audio_feat_seq.shape[1], :]
                else:
                    face_feat_seq = torch.cat(
                        (face_feat_seq, face_feat_seq[:, -min_gap_2:, :]), 1)

            face_fusion_seq = self.facial_fusion(torch.cat(
                (face_feat_seq, audio_feat_seq, emo_feat_seq, speaker_feat_seq, text_feat_seq), dim=2).reshape(-1, self.facial_fusion_dim))
            face_feat_seq = face_fusion_seq.reshape(*face_feat_seq.shape)
            in_data = torch.cat((in_data, face_feat_seq),
                                2) if in_data is not None else face_feat_seq
        ...
        output_hands = output_hands[:, :, :self.hidden_size] + \
            output_hands[:, :, self.hidden_size:]
        output_hands = self.out_hands(
            output_hands.reshape(-1, output_hands.shape[2]))
        decoder_outputs_hands = output_hands.reshape(
            in_data.shape[0], in_data.shape[1], -1)

        decoder_outputs_final = torch.zeros(
            (in_data.shape[0], in_data.shape[1], 141)).cuda()
        ...
        decoder_outputs_final[:, :,
                              84:141] = decoder_outputs_hands[:, :, 57:114]
