    ...
    def train(self, epoch):
        ...
        for its, dict_data in enumerate(self.train_loader):
            ...
            poses_feat, pose_mu, pose_logvar, recon_data = \
                self.model(None, tar_pose,
                           variational_encoding=self.variational_encoding)

            recon_loss = self.rec_loss(recon_data, tar_pose)  # 128*34*123
            recon_loss = torch.mean(recon_loss, dim=(1, 2))  # 128
            self.loss_meters['rec_l1'].update(
                torch.sum(recon_loss).item()*self.rec_weight)
            ...
            if self.vel_weight > 0:  # use pose diff
                ...
                vel_rec_loss = torch.mean(self.vel_loss(
                    recon_diff, target_diff), dim=(1, 2))
                self.loss_meters['vel_l1'].update(
                    torch.sum(vel_rec_loss).item()*self.vel_weight)
                recon_loss += (torch.sum(vel_rec_loss)*self.vel_weight)
            # KLD
            if self.variational_encoding:
                KLD = -0.5 * torch.sum(1 + pose_logvar -
                                       pose_mu.pow(2) - pose_logvar.exp())
                if epoch < 10:
                    KLD_weight = 0
                else:
            ...
            if self.grad_norm != 0 and "LSTM" in self.g_name:
...
#             logger.warning(total_norm)
            ...
            if its % self.log_period == 0:
                self.recording(epoch, its, its_len, self.loss_meters,
                               lr_g, 0, t_data, t_train, mem_cost)
            # if its == 1:break
        ...
        with torch.no_grad():
            ...
            for its, dict_data in enumerate(self.val_loader):
                ...
                if self.vel_weight > 0:  # use pose diff
                    ...
                    recon_diff = recon_data[:, 1:] - recon_data[:, :-1]
                    vel_rec_loss = torch.mean(self.vel_loss(
                        recon_diff, target_diff), dim=(0, 1, 2))
        ...
        with torch.no_grad():
            ...
            for its, dict_data in enumerate(self.test_loader):
                ...
                if "LSTM" in self.g_name or "multi_length" in self.notes:
                    poses_feat, pose_mu, pose_logvar, recon_data = \
                        self.model(**dict(pre_poses=None, poses=tar_pose,
                                   variational_encoding=self.variational_encoding))
                    out_final = (recon_data.cpu().numpy().reshape(-1,
                                 self.pose_dims) * self.std_pose) + self.mean_pose
                else:
                    for i in range(tar_pose.shape[1]//(self.pose_length)):
                        tar_pose_new = tar_pose[:, i*(self.pose_length):i*(
                            self.pose_length)+self.pose_length, :]
                        poses_feat, pose_mu, pose_logvar, recon_data = \
                            self.model(**dict(pre_poses=None, poses=tar_pose_new,
                                       variational_encoding=self.variational_encoding,))
                        out_sub = (recon_data.cpu().numpy(
