...
class BaseTrainer(object):
    def __init__(self, args):
        ...
        self.train_data = __import__(f"dataloaders.{args.dataset}", fromlist=[
                                     "something"]).CustomDataset(args, "train")
        self.train_loader = torch.utils.data.DataLoader(
            ...
            sampler=torch.utils.data.distributed.DistributedSampler(
                self.train_data) if self.ddp else None,
        )
        self.train_length = len(self.train_loader)
        logger.info(f"Init train dataloader success")

        self.val_data = __import__(f"dataloaders.{args.dataset}", fromlist=[
                                   "something"]).CustomDataset(args, "val")
        self.val_loader = torch.utils.data.DataLoader(
            ...
            drop_last=False,
            sampler=torch.utils.data.distributed.DistributedSampler(
                self.val_data) if self.ddp else None,
        )
        ...
        if self.ddp:
            ...
            self.model = torch.nn.DataParallel(
                getattr(model_module, args.g_name)(args), args.gpus).cuda()
        if self.rank == 0:
        ...
        if args.d_name is not None:
            if self.ddp:
                self.d_model = getattr(
                    model_module, args.d_name)(args).to(self.rank)
                self.d_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(
                    self.d_model, process_group)
                self.d_model = DDP(self.d_model, device_ids=[self.rank], output_device=self.rank,
                                   broadcast_buffers=False, find_unused_parameters=False)
            else:
                self.d_model = torch.nn.DataParallel(
                    getattr(model_module, args.d_name)(args), args.gpus).cuda()
            if self.rank == 0:
                logger.info(self.d_model)
                wandb.watch(self.d_model)
                logger.info(f"init {args.d_name} success")
            self.opt_d = create_optimizer(
                args, self.d_model, lr_weight=args.d_lr_weight)
        ...
        if args.e_name is not None:
            ...
            if self.ddp:
                self.eval_model = getattr(
                    eval_model_module, args.e_name)(args).to(self.rank)
            else:
            ...
            if self.ddp:
                self.eval_model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(
                    self.eval_model, process_group)
            ...
            for name, loss_meter in self.loss_meters.items():
                if "val" not in name:
                    if loss_meter.count > 0:
                        pstr += "{}: {:.3f}\t".format(loss_meter.name,
                                                      loss_meter.avg)
                        wandb.log({loss_meter.name: loss_meter.avg},
                                  step=epoch*self.train_length+its)
                        loss_meter.reset()
            pstr += "glr: {:.1e}\t".format(lr_g)
            pstr += "dlr: {:.1e}\t".format(lr_d)
            wandb.log({'glr': lr_g, 'dlr': lr_d},
                      step=epoch*self.train_length+its)
...
def main_worker(rank, world_size, args):
    ...
    trainer = __import__(f"{args.trainer}_trainer", fromlist=["something"]).CustomTrainer(
        args) if args.trainer != "base" else BaseTrainer(args)
    ...
    for epoch in range(args.epochs):
        ...
        if trainer.rank == 0:
            logger.info("Time info >>>>  elapsed: %.2f mins\t" % (
                epoch_time/60)+"remain: %.2f mins" % ((args.epochs/(epoch+1e-7)-1)*epoch_time/60))
        if trainer.ddp:
        ...
        if (epoch+1) % args.test_period == 0:
            if rank == 0:
                trainer.test(epoch)
                other_tools.save_checkpoints(os.path.join(
                    trainer.checkpoint_path, f"last_{epoch}.bin"), trainer.model, opt=None, epoch=None, lrs=None)
