...
class CustomTrainer(BaseTrainer):
    ...
    def __init__(self, args):
        ...
        self.rec_loss_rand = get_loss_func(
            "huber_loss", beta=0.05, reduction="none")
        if self.ddp:
    ...
    def train(self, epoch):
        ...
        for its,  dict_data in enumerate(self.train_loader):
            tar_pose, in_audio, in_word, in_id = dict_data["pose"], dict_data[
                "audio"], dict_data["word"], dict_data["id"]
            t_data = time.time() - t_start
            if self.ddp:
                tar_pose = tar_pose.to(self.rank)
                in_audio = in_audio.to(
                    self.rank) if self.audio_rep is not None else None
                in_word = in_word.to(
                    self.rank) if self.word_rep is not None else None
                in_id = in_id.long().to(self.rank) if self.speaker_id else None
                in_pre_pose = tar_pose.new_zeros(
                    (tar_pose.shape[0], tar_pose.shape[1], tar_pose.shape[2] + 1)).to(self.rank)
            else:
            ...
            # --------------------------- d training --------------------------------- #
            d_loss_final = 0
            out_pose, z, z_mu, z_logvar = self.model(
                **dict(pre_seq=in_pre_pose, in_audio=in_audio, in_word=in_word, in_id=in_id))
            # print(z,z_mu)
            if use_adv:
                ...
                d_loss_adv = torch.sum(-torch.mean(torch.log(out_d_real +
                                       1e-8) + torch.log(1 - out_d_fake + 1e-8)))
                d_loss_final += d_loss_adv
                self.loss_meters['dis'].update(
                    d_loss_final.item())  # we ignore batch_size here
            ...
            if use_adv:
                ...
                # self.adv_loss(out_d_fake, real_gt) # here 1 is real
            ...
            if self.speaker_id:
                ...
                out_pose_rand_vid, z_rand_vid, _, _ = self.model(
                    **dict(pre_seq=in_pre_pose, in_audio=in_audio, in_word=in_word, in_id=rand_vids))
                huber_value_rand = self.rec_loss_rand(
                    out_pose_rand_vid, out_pose)
                # print(self.rec_loss_rand.reduction, self.rec_loss_rand.beta)
                huber_value_rand = huber_value_rand.sum(dim=1).sum(dim=1)
                huber_value_rand = huber_value_rand.view(
                    huber_value_rand.shape[0], -1).mean(1)
                z_l1 = F.l1_loss(
                    z.detach(), z_rand_vid.detach(), reduction='none')
            ...
            if its % self.log_period == 0:
                self.recording(epoch, its, its_len, self.loss_meters,
                               lr_g, lr_d, t_data, t_train, mem_cost)
            # if its == 1:break
            ...
            for its, dict_data in enumerate(self.val_loader):
                ...
                if its == 0:
                    ...
                    latent_out_all = np.concatenate(
                        [latent_out_all, latent_out.detach().cpu().numpy()], axis=0)
                    latent_ori_all = np.concatenate(
                        [latent_ori_all, latent_ori.detach().cpu().numpy()], axis=0)
                huber_value = self.rec_loss(tar_pose, out_pose)
                huber_value *= self.rec_weight
